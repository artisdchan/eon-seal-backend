...
export class DashboardController {
    ...
    public serverInfo = async (req: Request, res: Response) => {
        ...
        try {
            ...
            //  Get all Cegel
            const queryAllCelgel = await GDB0101DataSource.manager.query('select SUM(p.money + ifnull(0, gs.segel) + s.segel) as amount from pc p left join guildinfo g on p.char_name = g.mastername left join guildstore gs  on g.name = gs.guildname LEFT JOIN store s on p.user_id = s.user_id ') as unknown as AllMoney[];
